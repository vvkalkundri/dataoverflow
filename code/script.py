...
def covid_vaccine(vaccination_status_files, user_meta_file, output_file):
    ...
    with open(output_file, 'wt') as out_file:
        ...
        tsv_writer.writerow(['state', 'city', 'vaccine', 'gender', 'unique_vaccinated_people'])
        ...
        for city, vaccine in city.items():
            for vaccine, gender in vaccine.items():
                for gender, count in gender.items():
                    with open(output_file, 'wt') as out_file:
                        tsv_writer = csv.writer(out_file, delimiter='\t')
                        tsv_writer.writerow([state, city, vaccine, gender, count])


# if __name__ == '__main__':
#     parent_directory = os.path.abspath(os.path.join(os.getcwd(), os.pardir))
#     vaccination_file_path = parent_directory + '/data/vaccination_status/' + 'vaccination_status_0.tsv'
#     user_information_file_path = parent_directory + '/data/'.strip() + 'user_*tsv'
#     print(vaccination_file_path)
#     print(user_information_file_path)
